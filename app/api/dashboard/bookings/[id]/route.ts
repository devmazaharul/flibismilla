    ...
    try {
        ...
        const details: BaggageDetail[] = bags.map((bag: any) => {
            const config = BAGGAGE_CONFIG[bag.type] || {
                label:
                    bag.type?.replace(/_/g, ' ').replace(/\b\w/g, (c: string) => c.toUpperCase()) ||
                    'Other Bag',
                icon: '📦',
                defaultWeight: 0,
            };
        ...
        return {
            ...
            totalWeight,
            totalWeightDisplay:
                totalWeight > 0 ? `${totalWeight}kg${hasAnyApprox ? ' approx' : ''} total` : 'N/A',
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    ...
    try {
        ...
        let duffelOrder: any;
        let finalDocuments = normalizeDocsForResponse(booking.documents || []);
        ...
        try {
            ...
            if (isCancelledRemote) {
                ...
                if (booking.status !== 'cancelled') {
                    ...
                    notesToAdd.push(
                        createAdminNote(`Auto-sync: Order cancelled on Duffel at ${cancelledAt}`),
                    );
                }
                ...
                if (rawDuffelDocs.length > 0) {
                    ...
                    const isLiveMode = booking.isLiveMode
                    if (booking.status !== 'issued' || (!hasLocalDocs && isLiveMode)) {
                            updates.status = 'issued';
                            notesToAdd.push(
                                createAdminNote(
                                    `Auto-sync: Ticket issued. PNR: ${newPNR} ${isLiveMode ?'': '(Test Mode - No PDF Expected)'}`,
                                ),
                            );
            ...
            if (needsUpdate) {
                ...
                finalDocuments = normalizeDocsForResponse(finalBooking.documents || []);
                finalPNR = finalBooking.pnr;
            } else {
                // No DB update needed — use Duffel docs for response if available
                if (rawDuffelDocs.length > 0) {
                    finalDocuments = normalizeDocsForResponse(rawDuffelDocs);
                }
                finalPNR = newPNR || finalPNR;
            }
        } catch (error: any) {
            console.error('⚠️ Duffel sync failed, serving from local database:', error.message);

            return NextResponse.json(
                {
                    success: false,
                    message: 'Failed to sync with airline. Showing local record.',
                    debug: error.message,
                    data: {
                        id: booking._id,
                        pnr: finalPNR,
        ...
        const docsForMatching: any[] =
            duffelOrder.documents && duffelOrder.documents.length > 0
                ? normalizeDocsForResponse(duffelOrder.documents)
                : finalDocuments;
        ...
        const passengers = duffelOrder.passengers.map((p: any) => {
            const ticketDoc =
                ...
                }) || (docsForMatching.length === 1 ? docsForMatching[0] : null);
            ...
            if (p.infant_passenger_id) {
                ...
                infantInfo = infant ? `${infant.given_name} ${infant.family_name}` : null;
            }
        ...
        if (finalBooking.airlineInitiatedChanges?.cancellation) {
            cancellationInfo = finalBooking.airlineInitiatedChanges.cancellation;
        }
