...
export async function PUT(req: Request) {
  try {
    ...
    if (isSameAsOld) {
      return NextResponse.json({ success: false, message: "New password cannot be the same as old" }, { status: 400 });
    }
    ...
    return NextResponse.json({ success: true, message: "Password changed successfully. Please log in again." }, { status: 200 });
