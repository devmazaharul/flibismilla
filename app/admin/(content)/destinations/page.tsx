...
import {
  ...
  Globe, Calendar, DollarSign, Tag, CheckCircle2, Star,
  LayoutGrid, TrendingUp, Map,
} from 'lucide-react';
          ...
          <div className="flex-1">
            <div className="flex gap-1">
              {Array.from({ length: 5 }).map((_, j) => (
                <div key={j} className="h-3 w-3 rounded bg-slate-100" />
              ))}
            </div>
          </div>
...
export default function DestinationsPage() {
  ...
  const activeCount = useMemo(() => destinations.filter((d) => d.isActive).length, [destinations]);
  const hiddenCount = useMemo(() => destinations.filter((d) => !d.isActive).length, [destinations]);
  const avgRating = useMemo(
    () =>
      totalDestinations
        ? (destinations.reduce((a, c) => a + c.rating, 0) / totalDestinations).toFixed(1)
        : '0',
      ...
      <div className="mx-auto flex max-w-[1400px] flex-col gap-6">
        ...
        >
          <div className="space-y-1.5">
            ...
            <p className="text-sm text-slate-500 max-w-lg">
              Manage travel locations, control visibility and update destination info.
            </p>
          </div>
        ...
        >
          ...
          <div className="flex items-center gap-2 text-[11px] text-slate-400">
            <span className="flex items-center gap-1.5 rounded-full bg-emerald-50 px-2.5 py-1 font-bold text-emerald-700 ring-1 ring-emerald-200/60">
              <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse" />
              {activeCount} Active
            </span>
            <span className="flex items-center gap-1.5 rounded-full bg-slate-100 px-2.5 py-1 font-bold text-slate-500 ring-1 ring-slate-200/60">
        ...
        >
          ...
          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-3.5">
            <div>
              <h2 className="text-sm font-semibold text-slate-900">All Destinations</h2>
              <p className="mt-0.5 text-[11px] text-slate-500">
                Showing{' '}
                <span className="font-semibold text-slate-700">{filteredDestinations.length}</span>{' '}
                of{' '}
                <span className="font-semibold text-slate-700">{totalDestinations}</span>{' '}
                destinations
              </p>
            </div>
          </div>
                        ...
                        <td className="px-5 py-4 text-right align-top relative">
                          ...
                          {openMenuId === item._id && (
                            ...
                            >
                              ...
                              <Link
                                href={`/admin/destinations/edit?id=${item._id}`}
                                onClick={() => setOpenMenuId(null)}
                              >
          ...
          {!loading && filteredDestinations.length > 0 && (
            <div className="flex items-center justify-between border-t border-slate-100 px-5 py-3">
              <p className="text-[11px] text-slate-400">
                ...
                {filteredDestinations.length !== 1 ? 's' : ''} displayed
              </p>
            ...
            <div className="relative bg-gradient-to-br from-rose-50 via-pink-50 to-orange-50 px-6 pt-8 pb-6">
              ...
              <div className="relative">
                ...
                <p className="mt-2 text-[13px] leading-relaxed text-slate-500">
                  This will permanently remove this destination. This action{' '}
          ...
          <div className="w-full max-w-lg max-h-[90vh] overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-slate-200/60 flex flex-col animate-in zoom-in-95 duration-200">
            ...
            <div className="relative h-56 w-full flex-shrink-0 overflow-hidden">
              ...
              <div className="absolute bottom-4 left-5 right-5">
                <span
                  className={`inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1 text-[10px] font-bold ring-1 mb-2 backdrop-blur-sm ${
                    viewData.isActive
                ...
                <div className="mt-1.5 flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="ml-1 text-[11px] text-white/70">
                      {viewData.rating} ({viewData.reviews})
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-xl border border-slate-200/60 bg-gradient-to-r from-slate-50 to-white p-3.5">
                  <div className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400 mb-1.5">
                    <DollarSign size={11} />
                    Currency
                  </div>
                  <p className="text-sm font-bold text-slate-900">{viewData.currency}</p>
                </div>
                <div className="rounded-xl border border-slate-200/60 bg-gradient-to-r from-slate-50 to-white p-3.5">
                  <div className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400 mb-1.5">
                    <Calendar size={11} />
                  ...
                  <p className="text-sm font-bold text-slate-900">{viewData.bestTime}</p>
                </div>
              </div>
              ...
              {/* Attractions */}
              <div>
                <h3 className="text-[11px] font-bold uppercase tracking-[0.12em] text-slate-400 mb-3 flex items-center gap-1.5">
                  <CheckCircle2 size={12} />
                  Key Attractions
                </h3>
                {viewData.attractions && viewData.attractions.length > 0 ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {viewData.attractions.map((attraction, i) => (
                      <div
                        key={i}
                      ...
                      >
                        <div className="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-lg bg-blue-50 text-blue-500">
                          <MapPin size={12} />
                        </div>
                        <span className="text-[12px] font-medium text-slate-700">{attraction}</span>
                      </div>
                    ))}
                  </div>
                ) : (
                  <p className="text-[12px] text-slate-400 italic">No attractions listed.</p>
                )}
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 border-t border-slate-100 px-6 py-4 bg-slate-50/50 flex-shrink-0">
              <Link href={`/admin/destinations/edit?id=${viewData._id}`} className="flex-1">
