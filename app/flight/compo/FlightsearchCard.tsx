    ...
    return (
        <div className="w-full max-w-6xl mx-auto">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className=" p-3 rounded-3xl   flex flex-col lg:flex-row gap-3 items-center w-full">
                                ...
                                {filteredFrom.map((airport, i) => {
                                    ...
                                    return (
                                        <div key={i} onClick={() => !isDisabled && handleSelect('from', airport)} className={`px-4 py-3 flex justify-between items-center rounded-lg transition-colors ${isDisabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:bg-rose-50'}`}>
                                            <div>
                                                <p className="text-sm font-bold text-gray-800">{airport.name}</p>
                                                <p className="text-[10px] text-left text-gray-500">{airport.city}</p>
                                            </div>
                ...
                {/* SEARCH BUTTON */}
                <Button type="submit" className="bg-red-700 hover:bg-red-800 w-full lg:w-auto px-12 h-12 rounded-2xl font-bold shadow-xl shadow-gray-900/10 text-lg transition-all hover:scale-105 active:scale-95 border-2 border-transparent">
