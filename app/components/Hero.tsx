...
import { FaSearch, FaCalendarAlt, FaPlaneDeparture, FaPlaneArrival, FaExchangeAlt } from "react-icons/fa";
...
const router=useRouter()
  ...
  const onSubmit: SubmitHandler<SearchInputs> = (data) => {
    ...
    router.push(`/packages/search?from=${data.from}&to=${data.to}&date=${data.date}`);
  };
    ...
    <section className="relative w-full min-h-[85vh] flex items-center justify-center">
      ...
      <div className={`relative z-10 w-full ${layout.container} flex flex-col items-center text-center mt-10`}>
        ...
        <form 
          ...
          className={`bg-white w-full max-w-6xl ${layout.radius.card} shadow-2xl p-4 md:p-6 grid grid-cols-1 md:grid-cols-12 gap-4 items-center relative`}
        >
          ...
          <div className="md:col-span-3 border-b md:border-b-0 md:border-r border-gray-200 px-4 py-2 relative group">
            <div className="flex items-center gap-2 mb-1 text-gray-400 text-sm">
              ...
              <label className="font-bold uppercase text-xs tracking-wider text-gray-500">From</label>
            </div>
            <input 
              {...register("from")}
              type="text" 
            ...
            <div className={`absolute bottom-0 left-4 right-4 h-[2px] transition-all duration-300 ${errors.from ? "bg-red-500" : "bg-transparent"}`} />
            {errors.to && <span className="text-[10px] text-red-500 absolute -bottom-3 left-4">{errors.to.message}</span>}
          </div>
          ...
          <div className="hidden md:flex absolute left-[25%] top-1/2 -translate-y-1/2 z-10 justify-center">
            <button 
              type="button"
              onClick={handleSwap}
              className={`bg-white border border-gray-200 rounded-full p-2 text-gray-500 hover:text-rose-600 hover:shadow-md transition-all duration-300 ${isSwapping ? "rotate-180" : ""}`}
          ...
          <div className="md:col-span-3 border-b md:border-b-0 md:border-r border-gray-200 px-4 py-2 relative md:pl-8">
            <div className="flex items-center gap-2 mb-1 text-gray-400 text-sm">
              ...
              <label className="font-bold uppercase text-xs tracking-wider text-gray-500">To</label>
            </div>
            <input 
              {...register("to")}
              type="text" 
            ...
            />
             <div className={`absolute bottom-0 left-4 right-4 h-[2px] transition-all duration-300 ${errors.to ? "bg-red-500" : "bg-transparent"}`} />
            {errors.to && <span className="text-[10px] text-red-500 absolute -bottom-3 left-8">{errors.to.message}</span>}
          </div>
          ...
          <div className="md:col-span-3 border-b md:border-b-0 md:border-r-0 px-4 py-2 relative">
             <div className="flex items-center gap-2 mb-1 text-gray-400 text-sm">
              ...
              <label className="font-bold uppercase text-xs tracking-wider text-gray-500">Journey Date</label>
            </div>
            <input 
              {...register("date")}
              type="date"
            ...
            />
             <div className={`absolute bottom-0 left-4 right-4 h-[2px] transition-all duration-300 ${errors.date ? "bg-red-500" : "bg-transparent"}`} />
            {errors.date && <span className="text-[10px] text-red-500 absolute -bottom-3 left-4">{errors.date.message}</span>}
          </div>
