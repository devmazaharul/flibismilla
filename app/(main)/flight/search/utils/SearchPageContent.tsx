        ...
        <div>
            ...
            <div className="grid grid-cols-2 gap-2">
                {[
                    ...
                    const count = stopCounts[stop.id] ?? 0;
                            ...
                            >
                                ...
                                <div className="w-full">
                                    <div className="flex items-baseline justify-between mb-1.5">
                                        ...
                                        >
                                            {airline.currency}{' '}
                                            {airline.minPrice.toLocaleString()}
                                        </span>
                                        {priceDiff > 0 && (
                                            <span
            ...
            {selectedAirlines.length > 0 && (
                <div className="mt-3 flex items-center gap-2 flex-wrap animate-in fade-in slide-in-from-top-1 duration-200">
                    ...
                    {selectedAirlines.map((name) => {
                        const airline = airlineStats.find(
                            (a) => a.name === name
                        );
                        return (
                            <button
...
function SearchPageContent() {
        ...
        const fetchFlights = async () => {
            ...
            try {
                let payload: Record<string, unknown> = {
                    ...
                    passengers: {
                        adults: parseInt(
                            searchParams.get('adt') || '1'
                        ),
                        children: parseInt(
                            searchParams.get('chd') || '0'
                        ),
                        infants: parseInt(
                            searchParams.get('inf') || '0'
                        ),
                    },
                    cabinClass:
    ...
    const filteredResults = useMemo(() => {
        ...
        if (selectedStops.length > 0) {
            res = res.filter((f) => {
                const stops = f.itinerary[0]?.stops ?? 0;
                if (
                    selectedStops.includes('direct') &&
                    stops === 0
                )
                    return true;
                if (
                    selectedStops.includes('1_stop') &&
                    stops === 1
                )
                    return true;
                if (
                    selectedStops.includes('2_stop') &&
                    stops === 2
                )
                    return true;
                if (
                    selectedStops.includes('2plus_stop') &&
                    stops > 2
                )
                    return true;
                return false;
        ...
        if (selectedAirlines.length > 0) {
            res = res.filter((f) =>
                selectedAirlines.includes(f.carrier.name)
            );
        }
        ...
        switch (sortBy) {
            ...
            case 'price_desc':
                res.sort(
                    (a, b) =>
                        b.price.finalPrice - a.price.finalPrice
                );
                break;
            case 'duration':
                res.sort((a, b) =>
    ...
    const { airlineCounts, stopCounts, uniqueAirlines } = useMemo(() => {
        ...
        return {
            airlineCounts: aCounts,
            stopCounts: sCounts,
            uniqueAirlines: Object.keys(aCounts),
        };
    }, [rawResults]);
            ...
            <section className="relative overflow-hidden bg-gray-950">
                ...
                <div className="max-w-6xl mx-auto px-4 relative z-10 pt-16 md:pt-24 pb-32 md:pb-40 text-center">
                    ...
                    <p className="text-gray-400 text-sm md:text-base max-w-xl mx-auto leading-relaxed">
                        Search, compare and book flights from hundreds of
                                        ...
                                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                <button
                                                    ...
                                                    className="
                                                        lg:hidden h-11 px-4 rounded-xl
