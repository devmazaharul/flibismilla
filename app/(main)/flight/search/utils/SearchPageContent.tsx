...
// ----------------------------------------------------------------------
// 🟢 COMPONENT: Airline Price Grid (Carousel)
// ----------------------------------------------------------------------
const AirlinePriceGrid = ({ 
    ...
    // Calculate Lowest Price per Airline
    const airlineStats = useMemo(() => {
        const stats: Record<string, { name: string, logo: string | null, minPrice: number, currency: string }> = {};
        ...
        <div className="w-full mb-8 relative group">
            <div className="flex items-center justify-between mb-3 px-1">
               <div className="flex items-center gap-2">
                   ...
                   <h3 className="text-xs font-bold uppercase tracking-widest text-slate-500">Compare Airlines</h3>
               </div>
               ...
               <div className="flex gap-2">
                   <button 
                        onClick={() => scroll('left')} 
                        className="p-1.5 rounded-full bg-white border hover:bg-slate-50 shadow-2xl shadow-gray-100 active:scale-95 transition-all cursor-pointer"
                   >
            ...
            {/* Scroll Container (Fixed Scrollbar Hiding) */}
            <div 
                ref={scrollRef}
                className="flex gap-3 overflow-x-auto pb-4 snap-x cursor-grab active:cursor-grabbing scroll-smooth [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]"
            >
                {airlineStats.map((airline) => {
                    ...
                    return (
                        <button
                            ...
                            className={`flex flex-col items-center justify-center min-w-[140px] p-4 rounded-2xl border transition-all duration-300 snap-start relative overflow-hidden cursor-pointer
                                ${isSelected 
                                    ? 'bg-slate-900 border-slate-900 text-white   scale-[1.02]' 
                                    : 'bg-white border-slate-200/60 text-slate-600 hover:border-gray-100 hover:shadow-2xl hover:shadow-gray-200/60'
                                }
                            `}
                        >
                            {/* Selection Indicator */}
                            ...
                            {/* Logo */}
                            <div className="h-10 w-10 mb-3 flex items-center justify-center bg-white rounded-full p-1.5 overflow-hidden shadow-sm border border-slate-100">
                                {airline.logo ? (
                            ...
                            {/* Name */}
                            <span className={`text-[10px] font-bold uppercase tracking-tight truncate w-full text-center mb-1.5
                            ...
                            <div className="flex flex-col items-center">
                                <span className={`text-[9px] font-semibold pb-1 uppercase ${isSelected ? 'text-slate-400' : 'text-emerald-600'}`}>Starting from</span>
...
function SearchPageContent() {
    ...
    useEffect(() => {
        const fetchFlights = async () => {
            ...
            if (rawResults.length === 0) setLoading(true);
                ...
                // Background refresh UI feedback
                setLoading(true);
                setTimeout(() => {
    ...
    const filteredResults = useMemo(() => {
        ...
        if (sortBy === 'price_asc') {
            ...
            // Simplified string compare for now
    ...
    const FilterContent = () => (
        <div className="space-y-8">
            {/* Stops */}
            <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3 block">Stops</label>
                <div className="grid grid-cols-2 gap-3">
                    {[
                        ...
                        <label key={stop.id} className={`flex flex-col items-center justify-center p-3 rounded-xl border-2 cursor-pointer transition-all duration-200 text-center relative
                            ${selectedStops.includes(stop.id) ? 'border-rose-600 bg-rose-50 text-rose-700' : 'border-slate-100 bg-white hover:border-slate-300 text-slate-600'}`}>
                            <span className="text-sm font-bold">{stop.label}</span>
                            <span className="text-[10px] font-medium opacity-60 mt-1">{stopCounts[stop.id as keyof typeof stopCounts]} flights</span>
                            <input type="checkbox" className="hidden" checked={selectedStops.includes(stop.id)} onChange={() => toggleStop(stop.id)} />
                            {selectedStops.includes(stop.id) && <div className="absolute top-1 right-1"><Check className="w-3 h-3 text-rose-600" /></div>}
                        </label>
                    ))}
                </div>
            </div>
            ...
            {/* Airlines */}
            <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3 block">Airlines</label>
                <div className="space-y-3">
                    {uniqueAirlines.map((airline) => (
                        <label key={airline} className="flex items-center justify-between cursor-pointer group hover:bg-slate-50 p-2 rounded-lg transition-colors -mx-2">
                            <div className="flex items-center gap-3">
                                <div className={`w-5 h-5 rounded-md border flex items-center justify-center transition-all ${selectedAirlines.includes(airline) ? 'bg-rose-600 border-rose-600' : 'border-slate-300'}`}>
                                    {selectedAirlines.includes(airline) && <Check className="w-3.5 h-3.5 text-white" />}
                                    <input type="checkbox" className="hidden" checked={selectedAirlines.includes(airline)} onChange={() => toggleAirline(airline)} />
                                </div>
                                <span className="text-sm font-medium text-slate-700">{airline}</span>
                            </div>
                            <span className="text-[10px] font-bold text-gray-700 bg-gray-100 px-2 py-0.5 rounded-full">{airlineCounts[airline]}</span>
                        </label>
                    ))}
                </div>
            </div>
        ...
        <div className="min-h-screen bg-[#F8FAFC] font-sans text-slate-900 pb-20">
            ...
            {showMobileFilter && (
                <div className="fixed inset-0 z-50 flex flex-col justify-end lg:hidden">
                    <div className="absolute inset-0 bg-black/50 backdrop-blur-sm transition-opacity" onClick={() => setShowMobileFilter(false)}></div>
                    <div className="bg-white rounded-t-3xl p-6 w-full max-h-[85vh] overflow-y-auto relative z-10 animate-in slide-in-from-bottom-10 duration-300">
                        <div className="flex items-center justify-between mb-6 border-b border-slate-100 pb-4">
                            <h3 className="text-lg font-black text-slate-800 flex items-center gap-2"><Filter className="w-5 h-5" /> Filter Flights</h3>
                            <button onClick={() => setShowMobileFilter(false)} className="p-2 bg-slate-100 rounded-full hover:bg-slate-200"><X className="w-5 h-5 text-slate-600" /></button>
                        </div>
                        <FilterContent />
                        <div className="mt-8 pt-4 border-t border-slate-100">
                            <button onClick={() => setShowMobileFilter(false)} className="w-full py-4 bg-slate-900 text-white font-bold rounded-xl shadow-lg shadow-slate-300">Apply Filters ({filteredResults.length})</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="relative pb-32 pt-10 px-4">
                <div className="absolute inset-0 z-0 overflow-hidden">
                    <img src="/e0b.jpg" alt="Flight Background" className="w-full h-full object-cover" />
                    <div className="absolute inset-0 bg-slate-900/80"></div>
                </div>
                <div className="max-w-7xl mx-auto relative z-10 text-center">
                    <h1 className="text-3xl md:text-5xl font-black text-white tracking-tight mb-2">Find Your Flight</h1>
                    <p className="text-slate-300 text-sm md:text-base">Compare prices from hundreds of airlines.</p>
                </div>
            </div>
                    ...
                    (loading || rawResults.length > 0 || searchParams.get('type')) && (
                        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
                            ...
                            <div className="hidden lg:block lg:col-span-1 bg-white rounded-[1.5rem] p-6 shadow-2xl shadow-gray-100 border border-gray-200/80 sticky top-4 max-h-[calc(100vh-2rem)] overflow-y-auto custom-scrollbar">
                                <div className="flex items-center gap-2 mb-6 pb-4 border-b border-slate-100 sticky top-0 bg-white ">
                            ...
                            <div className="col-span-1 lg:col-span-3">
                                {loading ? (
                                    ...
                                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                                        ...
                                        {/* 🟢 REUSABLE PRICE NOTICE */}
                                        <PriceValidityNotice offerId='' showTimer={false} />

                                        {/* 🚀 NEW: AIRLINE PRICE GRID (Carousel) */}
                                        <AirlinePriceGrid 
                                            offers={rawResults} 
                                        ...
                                        {/* Result Stats & Sorting */}
                                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-6">
                                            <div>
                                                <div className="flex items-center gap-3">
                                                    <h2 className="text-2xl font-bold text-slate-900">Found {filteredResults.length} Flights</h2>
                                                    <span className="px-2.5 py-1 rounded bg-gray-700 text-gray-100 text-[10px] font-bold uppercase tracking-wider">{searchParams.get('class') || 'Economy'}</span>
                                                </div>
                                            ...
                                            <div className="flex items-center gap-2 mt-4 md:mt-0">
                                                <button onClick={() => setShowMobileFilter(true)} className="lg:hidden p-3 bg-white border border-slate-200 rounded-xl text-slate-700 hover:bg-slate-50 transition-colors shadow-sm">
                                                    <SlidersHorizontal className="w-5 h-5" />
                                                </button>
                                                
                                                <div className="relative group">
                                                    <div className="absolute inset-y-0 right-3 flex items-center pointer-events-none"><ChevronDown className="w-4 h-4 text-slate-500" /></div>
                                                    <select
                                                        value={sortBy}
                                    ...
                                    <div className="text-center py-32 opacity-50">
                                        ...
                                        <h3 className="text-2xl font-black text-slate-800">No flights found</h3>
                                    </div>
                                )}
                            </div>
                        </div>
...
export default function FlightSearchPage() {
    return (
        <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><div className="w-8 h-8 border-4 border-rose-600 border-t-transparent rounded-full animate-spin"></div></div>}>
            <SearchPageContent />
        </Suspense>
    );
}