...
export default function RoundTripForm({ onSearch }: { onSearch: (params: URLSearchParams) => void }) {
    ...
    const handlePaxChange = useCallback((data: any) => {
        setFormData(prev => ({ ...prev, passengers: data.passengers, cabinClass: data.cabinClass }));
    }, []);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        // 🟢 FIX: Correctly sending all passenger types
    ...
    return (
        <form onSubmit={handleSubmit} className="w-full p-4 rounded-[1.5rem] relative z-20">
            ...
            <div className="flex flex-col xl:flex-row gap-3 xl:gap-2 items-center">
                ...
                {/* Locations */}
                <div className="flex flex-col md:flex-row w-full xl:w-[35%] items-center gap-2 md:gap-0 relative">
                ...
                {/* Departure Date */}
                <div className={`w-full xl:w-[15%] group relative h-[56px] bg-white border rounded-xl px-4 flex flex-col justify-center transition-all ${errors.departureDate ? 'border-red-500 ring-1 ring-red-500/20 bg-red-50' : 'border-slate-300 hover:border-rose-400 focus-within:ring-2 focus-within:ring-rose-500/20 focus-within:border-rose-500'}`}>
                    <div className={`absolute left-3 top-1/2 -translate-y-1/2 p-1.5 rounded-lg transition-colors ${errors.departureDate ? 'bg-red-100 text-red-500' : 'group-focus-within:bg-rose-50 group-focus-within:text-rose-600'}`}>
                        <Calendar className="w-4 h-4 text-rose-500" />
                    </div>
                    <div className="ml-10">
                        <label className={`text-[10px] font-bold uppercase tracking-widest block leading-tight ${errors.departureDate ? 'text-red-400' : 'text-slate-400'}`}>Departure</label>
                        <input 
                            type="date" 
                            min={today} 
                ...
                {/* Return Date */}
                <div className={`w-full xl:w-[15%] group relative h-[56px] bg-white border rounded-xl px-4 flex flex-col justify-center transition-all ${errors.returnDate ? 'border-red-500 ring-1 ring-red-500/20 bg-red-50' : 'border-slate-300 hover:border-rose-400 focus-within:ring-2 focus-within:ring-rose-500/20 focus-within:border-rose-500'}`}>
                    <div className={`absolute left-3 top-1/2 -translate-y-1/2 p-1.5 rounded-lg transition-colors ${errors.returnDate ? 'bg-red-100 text-red-500' : 'group-focus-within:bg-rose-50 group-focus-within:text-rose-600'}`}>
                        <Calendar className="w-4 h-4 text-rose-500" />
                    </div>
                    <div className="ml-10">
                        <label className={`text-[10px] font-bold uppercase tracking-widest block leading-tight ${errors.returnDate ? 'text-red-400' : 'text-slate-400'}`}>Return</label>
                        <input 
                            type="date" 
                ...
                <button 
                    type="submit" 
                    className="w-full xl:w-[15%] h-[56px] bg-rose-600 hover:bg-rose-700 text-white font-bold rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer active:scale-95 shadow-md shadow-rose-200"
                >
            ...
            {Object.keys(errors).length > 0 && (
                <div className="absolute -bottom-10 left-0 bg-red-100 border border-red-200 text-red-700 px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 animate-in slide-in-from-top-2">
                    ...
                    {errors.origin || errors.destination || errors.departureDate || errors.returnDate || "Please check all fields."}
                </div>
            )}
        </form>
    );
