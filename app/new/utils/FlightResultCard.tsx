...
import { 
  Plane,  AlertCircle, 
  ArrowRight, Clock, ChevronDown, Check, Filter, X, SlidersHorizontal, Info 
} from 'lucide-react';
...
interface FlightSegment {
  id: string; airline: string; airlineCode: string; logo: string | null;
  flightNumber: string; aircraft: string;
...
interface FlightItinerary {
  id: string; direction: string; totalDuration: string; stops: number;
  segments: FlightSegment[]; mainDeparture: FlightSegment['departure'];
  mainArrival: FlightSegment['arrival']; mainAirline: string; mainLogo: string | null;
}
...
const SmartFlightPath = ({ duration, stops }: { duration: string, stops: number }) => (
    <div className="flex flex-col items-center justify-center w-full px-2 min-w-[80px] md:min-w-[120px]">
        <div className="flex justify-between w-full mb-1.5">
            ...
            <span className={`text-[9px] md:text-[10px] font-bold ${stops === 0 ? 'text-emerald-600' : 'text-amber-600'}`}>
      ...
      <div className="flex flex-col lg:flex-row divide-y lg:divide-y-0 lg:divide-x divide-slate-100">
        ...
        <div className="flex-[3] p-5 flex flex-col justify-center gap-6">
          {flight.itinerary.map((leg, idx) => (
            <div key={idx} className="flex flex-col md:flex-row items-start md:items-center gap-4 md:gap-6">
              
              {/* Airline Info */}
              <div className="flex items-center gap-3 w-full md:w-[180px] shrink-0">
                  <div className="w-10 h-10 shrink-0 flex items-center justify-center bg-white rounded-xl border border-slate-100 shadow-sm p-1">
                     {leg.mainLogo ? <img src={leg.mainLogo} alt={leg.mainAirline} className="w-full h-full object-contain" /> : <Plane className="w-5 h-5 text-slate-300" />}
                  </div>
                  <div>
                      ...
                      <p className="text-[10px] font-semibold text-slate-400 mt-0.5 uppercase tracking-wide">
                        {leg.segments[0].airlineCode}{leg.segments[0].flightNumber} • {leg.segments[0].aircraft}
                      </p>
                  </div>
              </div>
        ...
        <div className="flex-1 bg-slate-50/50 p-5 flex flex-row lg:flex-col justify-between lg:justify-center items-center gap-4">
            <div className="text-left lg:text-center">
                ...
                <div className="flex items-baseline lg:justify-center gap-1 text-slate-900">
                    ...
                    <span className="text-2xl lg:text-3xl font-black tracking-tight">{flight.price.finalPrice.toLocaleString()}</span>
                </div>
            </div>

            <div className="flex flex-col gap-2 w-auto lg:w-full">
                <button className="px-6 py-2.5 bg-slate-900 hover:bg-black text-white rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-transform active:scale-95 shadow-lg shadow-slate-200">
                    Select <ArrowRight className="w-4 h-4" />
                </button>
                <button onClick={() => setIsExpanded(!isExpanded)} className="text-[10px] font-bold text-slate-400 hover:text-rose-600 flex items-center justify-center gap-1 mt-1 transition-colors cursor-pointer">
                    {isExpanded ? 'Hide Details' : 'View Details'} <ChevronDown className={`w-3 h-3 ${isExpanded ? 'rotate-180' : ''}`} />
                </button>
            </div>
        </div>
      </div>
            ...
            {flight.itinerary.map((leg, i) => (
                <div key={i} className="mb-6 last:mb-0">
                    <div className="flex items-center gap-3 mb-4">
                         <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">{leg.direction} Trip</span>
                                ...
                                <div className="bg-white p-4 rounded-xl border border-slate-200/80 shadow-2xl shadow-gray-100 flex flex-col gap-4">
                                    <div className="flex items-center justify-between pb-3 border-b border-slate-50">
                                        <div className="flex items-center gap-3">
                                            <img src={seg.logo || ''} className="w-8 h-8 object-contain" alt="logo" />
                                            <div>
                                                <p className="text-sm font-black text-slate-900">{seg.airline}</p>
                                                <p className="text-[10px] font-semibold text-slate-400">{seg.aircraft} • {seg.airlineCode}{seg.flightNumber}</p>
                                            </div>
                                        </div>
                                        <div className="text-right">
                                            <span className="text-[10px] font-bold bg-slate-100 text-slate-600 px-2 py-1 rounded-full block mb-1">{formatDuration(seg.duration)}</span>
                                            {/* 🟢 New Property: Cabin Class Name */}
                                            {seg.classType && <span className="text-[9px] font-bold text-slate-400 uppercase">{seg.classType}</span>}
                                        </div>
                                    </div>
                                    <div className="flex justify-between items-center">
                                        <div>
                                            <p className="text-lg font-black text-slate-900">{formatTime(seg.departure.time)}</p>
                                            <p className="text-xs  text-slate-500">{seg.departure.airport} ({seg.departure.code})</p>
                                        </div>
                                        <ArrowRight className="w-4 h-4 text-rose-300" />
                                        <div className="text-right">
                                            <p className="text-lg font-black text-slate-900">{formatTime(seg.arrival.time)}</p>
                                            <p className="text-xs  text-slate-500">{seg.arrival.airport} ({seg.arrival.code})</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
function SearchPageContent() {
  ...
  useEffect(() => {
    const fetchFlights = async () => {
        ...
        try {
            ...
            const res = await fetch('/api/new', { method: 'POST', body: JSON.stringify(payload) });
  ...
  const handleSearchPush = (params: URLSearchParams) => {
    setLoading(true); setRawResults([]);
    setTimeout(() => { resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }, 10);
  ...
  const filteredResults = useMemo(() => {
    ...
    else if (sortBy === 'duration') res.sort((a, b) => a.itinerary[0].totalDuration.localeCompare(b.itinerary[0].totalDuration));
    return res;
  }, [rawResults, sortBy]);
    ...
    <div className="min-h-screen bg-[#F8FAFC] font-sans text-slate-900">
      ...
      <div className="bg-slate-900 pt-10 pb-32 px-6 relative overflow-hidden">
         <div className="max-w-7xl mx-auto relative z-10 text-center">
            ...
            <p className="text-slate-400 text-sm md:text-base">Compare prices from hundreds of airlines.</p>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 -mt-24 relative z-20 mb-12">
        <div className="bg-white rounded-2xl shadow-xl shadow-slate-900/10 border border-slate-100 p-2">
            <div className="flex gap-1 p-2 border-b border-slate-100 mb-4 overflow-x-auto no-scrollbar">
                {tabs.map((tab) => (
                    <button key={tab.id} onClick={() => setActiveTab(tab.id as any)} className={`px-5 py-2 rounded-full text-xs font-bold uppercase tracking-wider whitespace-nowrap transition-all ${activeTab === tab.id ? 'bg-slate-900 text-white shadow-md' : 'text-slate-500 hover:bg-slate-50'}`}>
                        {tab.label}
                    </button>
                ))}
            </div>
            <div className="p-2 md:p-4">
                 ...
                 {activeTab === 'round_trip' && <RoundTripForm onSearch={handleSearchPush} />}
                 {activeTab === 'multi_city' && <MultiCityForm onSearch={handleSearchPush} />}
            </div>
        </div>
      </div>
      ...
      <div ref={resultsRef} className="scroll-mt-24 max-w-5xl mx-auto px-4 pb-20">
        ...
        {loading ? (
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-6">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => setShowMobileFilter(true)} className="lg:hidden p-2 bg-slate-100 rounded-xl text-slate-600 hover:bg-slate-200 transition-colors">
                ...
                <div className="space-y-5">
                    {filteredResults.map((f) => <FlightResultCard key={f.id} flight={f} />)}
                </div>
            </div>
        ) : (
            // Empty State
            <div className="text-center py-32 opacity-50">
                ...
                <p className="text-slate-400 mt-2">Select your destination to see flights.</p>
            </div>
        )}
      </div>
    </div>
