...
import { FlightSearchSkleton } from '../flight/search/compo/FlightSearchSkeleton';
...
function SearchPageContent() {
    ...
    useEffect(() => {
        const fetchFlights = async () => {
            ...
            try {
                let payload: any = {
                    type,
                    // 🟢 Fixed: Parsing all passengers for API payload
                    passengers: {
    ...
    const uniqueAirlines = useMemo(() => Array.from(new Set(rawResults.map((f) => f.carrier.name))), [rawResults]);
    ...
    const FilterContent = () => (
        <>
            <div className="mb-8">
                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3 block">Max Price: {priceRange[1].toLocaleString()}</label>
                <input type="range" min={0} max={maxPriceLimit} value={priceRange[1]} onChange={(e) => setPriceRange([0, parseInt(e.target.value)])} className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-rose-600" />
            </div>
            <div>
    ...
    return (
        <div className="min-h-screen bg-[#F8FAFC] font-sans text-slate-900 pb-20">
            ...
            <div className="bg-slate-900 pb-32 pt-10 px-4 relative">
                <div className="max-w-7xl mx-auto relative z-10 text-center">
                    <div className="flex justify-center md:justify-start gap-2 mb-8">
                        {tabs.map((tab) => (
                            <button key={tab.id} onClick={() => setActiveTab(tab.id as any)} className={`flex cursor-pointer items-center gap-2 px-6 py-3 rounded-full text-xs font-bold uppercase tracking-wider transition-all ${activeTab === tab.id ? 'bg-rose-600 text-white shadow-xl shadow-rose-900/40' : 'bg-slate-800/40 text-slate-400 hover:bg-slate-800'}`}>
                                {tab.label}
                            </button>
                        ))}
                    </div>
            ...
            <div className="max-w-7xl mx-auto px-4 -mt-24 relative z-30 mb-16">
                <div className="bg-white rounded-[2rem] shadow-2xl shadow-slate-200/50 p-2 md:p-4">
            ...
            <div className="max-w-7xl mx-auto px-4">
                {error ? (
                    <div className="bg-red-50 text-red-600 p-8 rounded-[2rem] border border-red-100 flex flex-col items-center justify-center gap-3 mt-8">
                        <AlertCircle className="w-8 h-8" /> <span className="font-semibold">{error}</span>
                    </div>
                ) : (
                    /* Main Layout Wrapper */
                                        ...
                                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-6">
                                            ...
                                            <div>
                                                <div className="flex items-center gap-3">
                                                    <h2 className="text-2xl font-bold flex items-center gap-2 text-slate-900">Found <p className='text-rose-600'>{filteredResults.length}</p> Flights</h2>
                                                    <span className="px-2.5 py-1 rounded bg-gray-700 text-slate-100 text-[10px] font-bold uppercase tracking-wider">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <button onClick={() => setShowMobileFilter(true)} className="lg:hidden p-2 bg-slate-100 rounded-xl text-slate-600 hover:bg-slate-200 cursor-pointer transition-colors">
                                                    <SlidersHorizontal className="w-5 h-5" />
                                                </button>
                                                <div className="bg-slate-100 p-1.5 rounded-xl flex items-center">
                                                    {[{ id: 'best', label: 'Best' }, { id: 'price', label: 'Cheapest' }, { id: 'duration', label: 'Fastest' }].map((opt) => (
                                                        <button key={opt.id} onClick={() => setSortBy(opt.id as any)} className={`px-4 md:px-6 py-2 rounded-lg text-sm font-bold cursor-pointer transition-all duration-200 ${sortBy === opt.id ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:bg-slate-200/50 hover:text-slate-700'}`}>
                                                            {opt.label}
                                                        </button>
                                                    ))}
                                                </div>
